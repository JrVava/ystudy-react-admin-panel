...
import { Edit2, Plus, Wrench, AlertCircle, Search, X, Trash2 } from "lucide-react";
...
export const ToolListPage: React.FC = () => {
  ...
  async function fetchTools(
    ...
    try {
      setLoading(true);
      setError(null);
      const res = await toolApi.getPaginated(page, limit, field, sort, search);
      if (res.success) {
        setTools(res.data || []);
  ...
  // Fetch tools on changes
  useEffect(() => {
    fetchTools(currentPage, rowsPerPage, sortField, sortOrder, debouncedSearchQuery);
  ...
  const columns = [
    {
      name: "Tool Title",
      selector: (row: any) => row.title || "Unnamed Tool",
      sortable: true,
      sortField: "title"
    },
    {
      ...
      cell: (row: any) => (
        <span
          style={{
            ...
            borderRadius: "6px",
            fontWeight: 600,
            background: row.mode === "free" ? "rgba(59, 130, 246, 0.15)" : "rgba(245, 158, 11, 0.15)",
            color: row.mode === "free" ? "var(--primary)" : "#f59e0b"
          }}
        >
          {row.mode || "Free"}
        </span>
    ...
    },
    {
      ...
      cell: (row: any) => (
        <div style={{ display: "flex", gap: "0.5rem" }}>
          <button
            onClick={() => navigate(`/tools/edit/${row._id}`)}
    ...
    <div className="animate-fade-in" style={{ width: "100%" }}>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Manage student application assistance widgets, calculators, and builders.</p>
        </div>
      ...
      <div className="panel-glass" style={{ padding: 0, overflow: "hidden" }}>
        <Table
          ...
          onSort={handleSort}
          noDataText="No tools found matching search criteria."
        />
      </div>
    </div>
  );
