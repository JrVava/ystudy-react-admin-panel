...
import { Edit2, Plus, BookOpen, AlertCircle, Search, X, Trash2 } from "lucide-react";
...
export const GuideListPage: React.FC = () => {
  ...
  async function fetchGuides(
    ...
    try {
      setLoading(true);
      setError(null);
      const res = await guideApi.getPaginated(page, limit, field, sort, search);
      if (res.success) {
        setGuides(res.data || []);
  ...
  // Fetch guides on changes
  useEffect(() => {
    fetchGuides(currentPage, rowsPerPage, sortField, sortOrder, debouncedSearchQuery);
  ...
  const columns = [
    {
      name: "Guide Title",
      selector: (row: any) => row.title || "Unnamed Guide",
      sortable: true,
      sortField: "title"
    },
    {
    ...
    },
    {
      ...
      cell: (row: any) => (
        <div style={{ display: "flex", gap: "0.5rem" }}>
          <button
            onClick={() => navigate(`/guides/edit/${row._id}`)}
    ...
    <div className="animate-fade-in" style={{ width: "100%" }}>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Manage curriculum resources, study templates, and student onboarding guides.</p>
        </div>
      ...
      <div className="panel-glass" style={{ padding: 0, overflow: "hidden" }}>
        <Table
          ...
          onSort={handleSort}
          noDataText="No guides found matching search criteria."
        />
      </div>
    </div>
  );
