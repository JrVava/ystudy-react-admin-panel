...
const Textarea = ({ label, value, onChange, placeholder, required = false }: { label: string; value: string; onChange: (e: React.ChangeEvent<HTMLTextAreaElement>) => void; placeholder?: string; required?: boolean }) => (
  <div className="form-group">
    <label className="form-label">{label}</label>
    <textarea
...
const BannerAdminPanel: React.FC = () => {
  ...
  const parsedColor = useMemo(
    () => parseColorAndOpacity(formData.background.bgColor),
    [formData.background.bgColor]
  );
            ...
            {activeTab === 'general' && (
              <div className="animate-fade-in">
                ...
                <div style={{ marginBottom: '24px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '16px', background: 'rgba(255, 255, 255, 0.02)', padding: '12px 16px', borderRadius: '10px', border: '1px solid var(--panel-border)' }}>
                    <div style={{ flexGrow: 1 }}>
                      ...
                      {(formData.rightCard?.items || []).map((item, index) => (
                        <div key={index} className="card-item-container">
                          <div className="card-item-header">
                            ...
                            <button type="button" onClick={() => removeItem(index)} style={{ background: 'none', border: 'none', color: 'var(--error)', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 600 }}>Remove</button>
                          </div>
                          ...
                          {['stats-highlight', 'grid-2x2'].includes(formData.rightCard.layoutType) && (
                            <div style={{ marginBottom: '8px' }}>
                              ...
                              <input type="text" value={item.value || ''} onChange={e => updateItem(index, 'value', e.target.value)} className="form-input card-item-input" />
                            </div>
                          )}
                          ...
                          {formData.rightCard.layoutType !== 'stats-highlight' && (
                            <div>
                              ...
                              <textarea value={item.description || ''} onChange={e => updateItem(index, 'description', e.target.value)} className="form-textarea card-item-textarea"></textarea>
                            </div>
                          )}
                        </div>
                      ))}
