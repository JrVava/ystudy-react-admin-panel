...
let activeToastEmitter:
  ((message: string, type: "success" | "error" | "warning" | "info", duration?: number) => void) | null = null;
...
const ToastItem: React.FC<{ toast: Toast; onClose: () => void }> = ({ toast, onClose }) => {
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", flexShrink: 0 }}>{getIcon()}</div>
      <div style={{ flexGrow: 1, fontSize: "0.85rem", fontWeight: 500, lineHeight: 1.4 }}>{toast.message}</div>
      <button
        onClick={onClose}
        style={{
          background: "none",
